...
def test_compute_epr_single_output():
    ...
    scores = detector.compute_epr([output], return_tokens=False)
    assert len(scores) == 1
...
def test_compute_epr_multiple_outputs():
    ...
    scores = detector.compute_epr(outputs, return_tokens=False)
    assert len(scores) == 3
...
def test_compute_epr_with_token_scores():
    ...
    seq_scores, token_scores = detector.compute_epr([output], return_tokens=True)
...
def test_compute_epr_high_vs_low_confidence():
    ...
    scores = detector.compute_epr([high_conf_output, low_conf_output], return_tokens=False)
